...
Multica 在首次启动时会自动生成 32 字节加密密钥并存入数据库，已保存的 Bot token 就用它加密，因此自托管部署不需要自己准备密钥。只有想自带密钥时才需要设置：

```dotenv
MULTICA_TELEGRAM_SECRET_KEY=<base64 编码的 32 字节密钥>
```

可用 `openssl rand -base64 32` 生成。该覆盖只作用于 Telegram 这一家；自己设置的密钥需要长期保存，丢失或轮换后，用它加密的 Bot token 无法解密，需要逐个重新连接。
