...
<Callout type="warn">
**セルフホスト版 Multica 専用です。** この連携は Multica を**自分でデプロイしている**場合のみ利用でき、Multica Cloud では提供されません。ここでの「セルフホスト」は **Multica 自体**のセルフホストを意味します。通常は独自ネットワーク内の Git インスタンスへアクセスするために使用します。サーバー運用者が `MULTICA_VCS_INTEGRATION_ENABLED=true` を設定して機能を有効にする必要があります。それまでは、設定 → 連携にこのセクションは表示されません。
</Callout>
...
サーバーは保存する認証情報を、初回起動時に自動生成してデータベースに保管する 32 バイトの鍵で暗号化します。鍵を設定しなくても接続フォームは使えます。自分の鍵を指定したい場合のみ、base64 でエンコードした 32 バイトの鍵を設定してください。
...
スイッチは機能を提供するかどうかを決めます。キーは自動生成された鍵をこの連携についてだけ置き換えるもので、接続、Webhook、ローテーションはいずれも実際に解決されたキーを使います。自分の鍵を指定した場合は継続して保管してください。その鍵を差し替えたり紛失したりすると、その鍵で暗号化済みの認証情報を復号できません。
