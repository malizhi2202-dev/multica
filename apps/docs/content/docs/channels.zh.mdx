...
Multica 在首次启动时自动生成一个 32 字节加密密钥并保存到数据库中，加密已保存的 Bot 凭据用的就是它，因此自托管部署无需任何前置配置即可开放连接入口。只有想自带密钥时才需要设置以下变量：
...
每个变量只覆盖对应平台的密钥，不设置不会影响其他平台。生成、保存和轮换方法见[环境变量](/environment-variables)。Multica Cloud 已完成这项配置。
