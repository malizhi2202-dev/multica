...
until then the section does not appear in Settings → Integrations.
</Callout>
...
The server encrypts stored credentials with a 32-byte key that it generates on first startup and keeps in the database, so no key setup is needed before the connect form works. Set one only to bring your own key:
...
The switch decides whether the feature is offered. `MULTICA_VCS_SECRET_KEY` only replaces the generated key for this integration, and connect, webhook, and rotation use whichever key the deployment resolves — so keep a key you bring long-term: credentials sealed with a replaced or lost key can no longer be decrypted.
