...
Multica は初回起動時に 32 バイトの認証情報暗号化キーを自動生成してデータベースに保存するため、以下の `MULTICA_*_SECRET_KEY` はいずれも任意のオーバーライドです。自分のキーを指定したい場合だけ設定してください。各キーは対象の連携にのみ影響します。
...
| Lark | `MULTICA_LARK_SECRET_KEY` | Lark の base64 エンコードされた 32 バイトの資格情報暗号化キー（任意のオーバーライド） |
| Slack | `MULTICA_SLACK_SECRET_KEY` | Slack の base64 エンコードされた 32 バイトの token 暗号化キー（任意のオーバーライド） |
| Telegram | `MULTICA_TELEGRAM_SECRET_KEY` | Telegram の base64 エンコードされた 32 バイトの Bot token 暗号化キー（任意のオーバーライド） |
...
| セルフホスト Git | `MULTICA_VCS_SECRET_KEY` | base64 エンコードされた 32 バイトの暗号化キー（`openssl rand -base64 32`）の任意のオーバーライド。機能の提供可否は上のスイッチが決めます |
| Plugins | `MULTICA_PLUGIN_SECRET_KEY` | 保存済み secret と surface 起動 URL を暗号化する base64 形式の 32 バイト鍵。Plugin だけ別鍵にしたい場合に設定します（任意） |
