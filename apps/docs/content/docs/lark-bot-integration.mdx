...
Multica Cloud needs no extra configuration, and neither does a self-hosted deployment: stored Bot credentials are encrypted with a 32-byte key that the server generates on first startup and keeps in its database. Set this key only to bring your own:
...
Restart the API server after setting it, then connect the Bot from the agent's **Integrations** page. The override covers Lark alone. Keep a key you set yourself long-term: if it is rotated or lost, the credentials it sealed can no longer be decrypted and the Bot must be reconnected.
