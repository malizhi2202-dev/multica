...
Multica Cloud users can skip this section, and a self-hosted deployment needs no key of its own either: the stored DingTalk `AppSecret` is encrypted with a 32-byte key that Multica generates on first startup and keeps in its database. Set this key only to bring your own:
...
Restart the API service after setting it, then connect DingTalk. The override covers DingTalk alone. Keep a key you set yourself long-term: if it is changed or lost, the tokens it sealed cannot be decrypted and the connection must be set up again.
