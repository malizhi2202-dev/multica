...
Multica は初回起動時に 32 バイトの暗号化キーを自動生成してデータベースに保存し、保存された Bot の認証情報はそのキーで暗号化されるため、接続エンドポイントを使うために事前の設定は不要です。自分のキーを使いたい場合のみ、以下を設定してください。
...
各キーは対象プラットフォームのキーだけを置き換え、未設定の他のプラットフォームには影響しません。生成、保管、ローテーションの方法は[環境変数](/environment-variables)を参照してください。Multica Cloud ではすでに設定済みです。
