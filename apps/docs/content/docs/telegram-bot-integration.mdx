...
Multica encrypts stored Bot tokens with a 32-byte key that it generates on first startup and keeps in its database, so a self-hosted deployment needs no key of its own. Set this one only to bring your own:

```dotenv
MULTICA_TELEGRAM_SECRET_KEY=<base64-encoded 32-byte key>
```

Generate one with `openssl rand -base64 32`. The override covers Telegram alone, and a key you set yourself has to be kept long-term: losing or rotating it makes the tokens it sealed unreadable and requires reconnecting each Bot.
