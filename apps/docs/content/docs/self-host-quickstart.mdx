...
2. Generate a random `JWT_SECRET`, PostgreSQL password, and `MULTICA_VCS_SECRET_KEY` (an optional override of the encryption key used by self-hosted Git integrations, which Multica generates itself when unset)
