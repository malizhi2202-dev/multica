...
Multica は初回起動時に 32 バイトの暗号化キーを自動生成してデータベースに保管し、保存された Bot token はそのキーで暗号化されるため、セルフホスト環境で独自にキーを用意する必要はありません。自分のキーを指定したい場合のみ設定してください。

```dotenv
MULTICA_TELEGRAM_SECRET_KEY=<base64-encoded 32-byte key>
```

`openssl rand -base64 32` で生成できます。このキーが適用されるのは Telegram だけで、自分で設定したキーは継続して保管する必要があります。連携リンクは `MULTICA_APP_URL`（未設定時は `FRONTEND_ORIGIN`）を使用します。サーバーから `api.telegram.org` への HTTPS 接続も必要で、標準の `HTTPS_PROXY` / `NO_PROXY` を利用できます。
