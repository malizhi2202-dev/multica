...
Multica generates its 32-byte credential-encryption key on first startup and stores it in the database, so each `MULTICA_*_SECRET_KEY` below is an optional override: set it only to bring your own key, and it applies to that integration alone.
...
| Lark | `MULTICA_LARK_SECRET_KEY` | Optional override of Lark's Base64-encoded 32-byte credential encryption key |
| Slack | `MULTICA_SLACK_SECRET_KEY` | Optional override of Slack's Base64-encoded 32-byte token encryption key |
| Telegram | `MULTICA_TELEGRAM_SECRET_KEY` | Optional override of Telegram's Base64-encoded 32-byte Bot token encryption key |
...
| Self-hosted Git | `MULTICA_VCS_SECRET_KEY` | Optional Base64-encoded 32-byte encryption key override (`openssl rand -base64 32`); the switch above, not this key, decides whether the feature is offered |
| Plugins | `MULTICA_PLUGIN_SECRET_KEY` | Optional Base64-encoded 32-byte key for stored secrets and encrypted surface launch URLs; set it to keep Plugin secrets on their own key |
