...
Multica 在首次启动时自动生成 32 字节凭据加密密钥并保存到数据库中，所以下面每个 `MULTICA_*_SECRET_KEY` 都是可选覆盖：只有想自带密钥时才设，而且只作用于自己那一家集成。
...
| 飞书 | `MULTICA_LARK_SECRET_KEY` | 可选覆盖项，覆盖飞书这一家使用的 base64 编码 32 字节凭据加密密钥 |
| Slack | `MULTICA_SLACK_SECRET_KEY` | 可选覆盖项，覆盖 Slack 这一家使用的 base64 编码 32 字节 token 加密密钥 |
| Telegram | `MULTICA_TELEGRAM_SECRET_KEY` | 可选覆盖项，覆盖 Telegram 这一家使用的 base64 编码 32 字节 Bot token 加密密钥 |
...
| 自托管 Git | `MULTICA_VCS_SECRET_KEY` | 可选覆盖项，base64 编码的 32 字节加密密钥（`openssl rand -base64 32`）；该功能是否开放只由上面的开关决定 |
| 插件 | `MULTICA_PLUGIN_SECRET_KEY` | 可选覆盖项，base64 编码的 32 字节密钥，用于加密已存 secret 和 surface 启动 URL；只有想让插件单独用一把密钥时才设 |
