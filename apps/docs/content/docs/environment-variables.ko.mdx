...
Multica는 첫 시작 시 32바이트 자격 증명 암호화 키를 자동 생성해 데이터베이스에 저장하므로, 아래 `MULTICA_*_SECRET_KEY`는 모두 선택적인 대체 값입니다. 자체 키를 쓰려는 경우에만 지정하세요. 각 키는 해당 연동에만 적용됩니다.
...
| Feishu | `MULTICA_LARK_SECRET_KEY` | Feishu의 base64로 인코딩한 32바이트 자격 증명 암호화 키 (선택적 대체) |
| Slack | `MULTICA_SLACK_SECRET_KEY` | Slack의 base64로 인코딩한 32바이트 token 암호화 키 (선택적 대체) |
| Telegram | `MULTICA_TELEGRAM_SECRET_KEY` | Telegram의 base64로 인코딩한 32바이트 Bot token 암호화 키 (선택적 대체) |
...
| 자체 호스팅 Git | `MULTICA_VCS_SECRET_KEY` | base64로 인코딩한 32바이트 암호화 키(`openssl rand -base64 32`)의 선택적 대체. 기능 제공 여부는 위 스위치가 결정합니다 |
| Plugins | `MULTICA_PLUGIN_SECRET_KEY` | 저장된 secret과 surface 실행 URL을 암호화하는 base64 인코딩 32바이트 키. Plugins만 별도 키를 쓰려면 지정합니다 (선택) |
