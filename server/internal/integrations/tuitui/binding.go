...
// This file is the Tuitui user-binding token flow: an unbound Tuitui user who
// messages the bot gets a "link your account" prompt (minted here, delivered by
// the outbound replier once one exists), clicks through to the in-product redeem
// page, and their Tuitui user account is bound to their Multica account. It
// mirrors dingtalk.BindingTokenService and runs on the generic channel_*
// queries with channel_type='tuitui' — no Tuitui-specific tables exist.
...
var (
	...
	ErrBindingTokenInvalid = errors.New("tuitui: binding token invalid or expired")
	// ErrBindingAlreadyAssigned: this Tuitui account is already bound to a
	// different Multica user (account transfer must go through explicit unbind).
	ErrBindingAlreadyAssigned = errors.New("tuitui: user id is already bound to a different user")
	// ErrBindingNotWorkspaceMember: the redeemer is not a member of the token's
	// workspace. Translated to 403 at the HTTP boundary.
	ErrBindingNotWorkspaceMember = errors.New("tuitui: redeemer is not a workspace member")
)
...
// BindingTokenService mints and redeems Tuitui binding tokens. Redemption is
...
// Mint creates a single-use binding token for (installation, tuituiUserID) and
// returns the raw secret + expiry. The raw value must be delivered over Tuitui
// (the platform carries the link over its own TLS) and never logged.
func (s *BindingTokenService) Mint(ctx context.Context, workspaceID, installationID pgtype.UUID, tuituiUserID string) (BindingToken, error) {
...
// RedeemAndBind atomically consumes a raw token and binds the Tuitui account to
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multicaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
		return RedeemedBindingToken{}, errors.New("tuitui: BindingTokenService missing TxStarter")
	}
	...
	if row.ChannelType != string(TypeTuitui) {
		// Consume and bind share this transaction, so returning here rolls the
		// consume back. A token from another adapter must never create a Tuitui
	...
	return RedeemedBindingToken{
		WorkspaceID:    row.WorkspaceID,
		InstallationID: row.InstallationID,
		TuituiUserID:   row.ChannelUserID,
	}, nil
}
